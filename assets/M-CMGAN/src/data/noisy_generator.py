...
def generate_noisy_data_db(clean_dir, noise_dir, output_base_dir, snr_range):
    ...
    for snr_db in snr_range:
        ...
        for clean_file in clean_files:
            ...
            if noise_waveform.size(1) > clean_waveform.size(1):
                # 计算随机起始位置
                max_start_idx = noise_waveform.size(1) - clean_waveform.size(1)
                start_idx = torch.randint(0, max_start_idx + 1, (1,)).item()
                noise_waveform = noise_waveform[:, start_idx:start_idx + clean_waveform.size(1)]
            else:
                # 如果噪声波形不够长（理论上不会发生，因为前面已经扩展了），使用原来的方法
                noise_waveform = noise_waveform[:, :clean_waveform.size(1)]
...
noise_dir = '/home/xyj/NoiseX-92/non-stationary/'
output_dir = os.path.join('/home/xyj/datasets/english/vb_Lsnr',)  #f"SNR:{snr_range}"
