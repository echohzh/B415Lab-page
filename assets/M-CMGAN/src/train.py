...
from models.generator_cmgan import TSCNet
from utils import *
...
parser.add_argument("--cut_len", type=int, default=16000*2, help="cut length, default is 2 seconds in denoise "
                                                                 "and dereverberation")
# parser.add_argument("--data_dir", type=str, default='/home/dataset/THCHS-30/data_thchs30/',
#                     help="dir of dataset")
# parser.add_argument("--noisy_dir", type=str, default='/home/xyj/datasets/chinese/',
...
parser.add_argument("--data_dir", type=str, default='/home/xyj/datasets/DNS-30h/',
                    help="dir of VCTK-DEMAND dataset")
parser.add_argument("--noisy_dir", type=str, default='/home/xyj/datasets/DNS-30h/',
                    help="dir of noisy data")
parser.add_argument("--save_model_dir", type=str, default='/home/xyj/Experiments/CMG-v1/src/ckpt_cmganDNS/',
...
class Trainer:
    ...
    def train_step(self, batch):
        ...
        noisy_spec = power_compress_pha(noisy_spec)[0]
        clean_spec, p_c = power_compress_pha(clean_spec)[0], power_compress_pha(clean_spec)[1]
        clean_real = clean_spec[:, 0, :, :].unsqueeze(1)
        clean_imag = clean_spec[:, 1, :, :].unsqueeze(1)
        est_real, est_imag = self.model(noisy_spec.permute(0, 1, 3, 2))
        ...
        current_size = predict_fake_metric.shape[0]

        # 检查当前大小与目标批量大小的关系
        if current_size < args.batch_size:
            # 如果当前尺寸小于批量大小，则使用填充（1）进行补充
            padding_tensor = torch.ones(args.batch_size - current_size, *predict_fake_metric.shape[1:]).to(
                predict_fake_metric.device)
            predict_fake_metric = torch.cat((predict_fake_metric, padding_tensor), dim=0)
        gen_loss_GAN = F.mse_loss(predict_fake_metric.flatten(), one_labels.float()) \
        loss_mag = F.mse_loss(est_mag, clean_mag)
        loss_ri = F.mse_loss(est_real, clean_real) + F.mse_loss(est_imag,
                                                                clean_imag)  # - F.mse_loss(est_real, noise_real))#/torch.sqrt(torch.sum(noise_real**2)) + \
        # loss_ip, loss_gd, loss_iaf = phase_losses(p_c, denoised_phase.permute(0, 3, 2, 1).squeeze(-1))
        # loss_pha = loss_ip + loss_gd + loss_iaf
        length = est_audio.size(-1)

        loss = args.loss_weights[0] * loss_ri + args.loss_weights[1] * loss_mag + args.loss_weights[2] * time_loss + args.loss_weights[4] * gen_loss_GAN
        loss.backward()
        ...
        if pesq_score is not None:
            ...
            # print(predict_enhance_metric.shape, predict_max_metric.shape)
            if predict_max_metric.shape[0] < args.batch_size:
                padding_tensor = torch.ones(args.batch_size - predict_max_metric.shape[0],1).to(predict_max_metric.device)
                predict_max_metric = torch.cat((predict_max_metric, padding_tensor), dim=0)
            elif predict_max_metric.shape[0] > args.batch_size:
                predict_max_metric = predict_max_metric[:args.batch_size]

            clean_loss = F.mse_loss(predict_max_metric.flatten(), one_labels)
            if predict_enhance_metric.shape[0] < pesq_score.shape[0]:
                padding_tensor = torch.ones(pesq_score.shape[0] - predict_enhance_metric.shape[0],1).to(predict_enhance_metric.device)
                predict_enhance_metric = torch.cat((predict_enhance_metric, padding_tensor), dim=0)
            elif predict_enhance_metric.shape[0] > pesq_score.shape[0]:
                predict_enhance_metric = predict_enhance_metric[:pesq_score.shape[0]]
    ...
    def test_step(self, batch):
        ...
        # 检查当前大小与目标批量大小的关系
        if current_size < args.batch_size:
            padding_tensor = torch.ones(args.batch_size - current_size, 1).to(
                predict_fake_metric.device)
            predict_fake_metric = torch.cat((predict_fake_metric, padding_tensor), dim=0)
    ...
    def train(self):
        ...
        for epoch in range(max(0, last_epoch),args.epochs):
            ...
            for idx, batch in enumerate(self.train_ds):
                ...
                template = 'time {}, Epoch {}, Step {}, loss: {}, disc_loss: {}'
                if (step % self.args.log_interval) == 0:
                    logging.info(template.format(spend_time, epoch, step, loss, disc_loss))
            ...
            path = os.path.join(args.save_model_dir,
                                'epoch' + str(epoch) + '-pesq:' + str(valid_pesq)[:5] + '-loss_sum:' + str(
                                    gen_loss[0] + gen_loss[1])[:6] + ':' + '-g:' + str(gen_loss[0])[:5] + '-d:' + str(
                                    gen_loss[1])[:5])
            ...
            if (epoch + 1) % args.save_epoch == 0:
                ...
                # 保存 valid_loss
                valid_loss_path = os.path.join(loss_history_dir,
                                               f'valid_loss_epoch_{epoch}_avg_{valid_loss_avg:.5f}.json')
                with open(valid_loss_path, 'w') as f:
                    json.dump({'valid_loss': self.loss_valid}, f)
                # 保存 valid_pesq
                train_avg_pesq = sum(self.pesq_history) / len(self.pesq_history)
                valid_pesq_path = os.path.join(loss_history_dir,
                                               f'valid_pesq_epoch_{epoch}_avg_{train_avg_pesq:.5f}.json')
...
def main():
    ...
    train_ds, test_ds = dataloader.load_base_data(args.data_dir, args.noisy_dir, args.batch_size, 10, args.cut_len) #用voicebank数据集训练
    # train_ds, test_ds = dataloader.load_data(args.data_dir, args.batch_size, 10, args.cut_len)   #其他数据集训练
