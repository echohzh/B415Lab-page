...
#     def __init__(self,h, out_channel=2):
...
#             nn.InstanceNorm2d(h.dense_channel, affine=True),
#         )
#         self.conv = nn.Conv2d(h.dense_channel, out_channel, (1, 2))
#
#     def forward(self, x):
#         x = self.dense_block(x)
...
#         self.complex_decoder = ComplexDecoder(h, out_channel=2)
#     def forward(self, noisy_amp, noisy_pha): # [B, F, T]
#         x_real = noisy_amp * torch.cos(noisy_pha)
#         x_imag = noisy_amp * torch.sin(noisy_pha)
#         noisy_com = torch.stack((x_real, x_imag), dim=1)# [B, 2, F, T]
#         x = torch.cat((noisy_amp.unsqueeze(1), noisy_com,noisy_pha.unsqueeze(1)), dim=1).permute(0, 1, 3, 2) # [B, 4, T, F]
#         # x = torch.cat((x, noisy_com), dim=1) # [B, 4, T, F]
...
#         complex_out = self.complex_decoder(x).permute(0,1,3,2)
#         denoised_com = torch.stack((denoised_amp*torch.cos(denoised_pha)+complex_out[:, 0, :, :],
#                                     denoised_amp*torch.sin(denoised_pha)+complex_out[:, 1, :, :]), dim=-1)
#
...
import torch.nn as nn
import numpy as np
# from MPmodels.conformer import ConformerBlock as TransformerBlock
from MPmodels.transformer import TransformerBlock
from pesq import pesq
from joblib import Parallel, delayed
...
class TSTransformerBlock(nn.Module):
    def __init__(self, h):
        super(TSTransformerBlock, self).__init__()
        self.h = h
        # self.time_transformer = TransformerBlock(dim=h.dense_channel, n_heads=4)
        # self.freq_transformer = TransformerBlock(dim=h.dense_channel, n_heads=4)
        self.time_transformer = TransformerBlock(d_model=h.dense_channel, n_heads=4)
        self.freq_transformer = TransformerBlock(d_model=h.dense_channel, n_heads=4)
