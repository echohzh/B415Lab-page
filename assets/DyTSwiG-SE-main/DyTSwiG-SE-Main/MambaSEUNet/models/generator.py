...
class MambaSEUNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.num_tscblocks = cfg['model_cfg']['num_tsblock'] if cfg['model_cfg']['num_tsblock'] is not None else 4  # default tfmamba: 4
        ...
        self.TSMamba1_encoder = nn.ModuleList([TFMambaBlock(cfg, dim[0]) for _ in range(self.num_tscblocks[0])])

        self.down1_2 = Downsample(dim[0], dim[1])

        self.patch_embed_encoder_level2 = Patch_Embed_stage(dim[1], dim[1])

        self.TSMamba2_encoder = nn.ModuleList([TFMambaBlock(cfg, dim[1]) for _ in range(self.num_tscblocks[1])])

        self.down2_3 = Downsample(dim[1], dim[2])

        self.patch_embed_middle = Patch_Embed_stage(dim[2], dim[2])

        self.TSMamba_middle = nn.ModuleList([TFMambaBlock(cfg, dim[2]) for _ in range(self.num_tscblocks[2])])
        ...
        self.TSMamba2_decoder = nn.ModuleList([TFMambaBlock(cfg, dim[1]) for _ in range(self.num_tscblocks[1])])
        ...
        self.TSMamba1_decoder = nn.ModuleList([TFMambaBlock(cfg, dim[0]) for _ in range(self.num_tscblocks[0])])

        # 幅度
        self.mag_patch_embed_refinement = Patch_Embed_stage(dim[0], dim[0])

        self.mag_refinement = nn.ModuleList([TFMambaBlock(cfg, dim[0]) for _ in range(self.num_tscblocks[0])])
        ...
        self.pha_refinement = nn.ModuleList([TFMambaBlock(cfg, dim[0]) for _ in range(self.num_tscblocks[0])])
