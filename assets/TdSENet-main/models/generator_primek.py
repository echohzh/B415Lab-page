...
from util import get_padding_2d, LearnableSigmoid_2d
...
class AdaptiveDecoder(nn.Module):
    def __init__(self, h, num_features, f_types, num_channel=64, ):  # out_channel=[M,P,C]
        ...
        self.mask_conv = nn.Conv2d(num_channel, 1, (1, 1))
        ...
        self.weights = [nn.Parameter(torch.rand(1), requires_grad=True).cuda(),
                        nn.Parameter(torch.rand(1), requires_grad=True).cuda()]
        if f_types == "C":
            self.complex_conv = nn.Conv2d(num_channel, 2, (1, 2))
        elif f_types == "P":
            self.phase_conv = nn.modules.ModuleList([nn.Conv2d(num_channel, 1, (1, 1)) for i in range(2)])
    ...
    def forward(self, x, f_types):
        ...
        if f_types == "C":
            ...
            x_mask = self.mask_act(self.mask_conv(x * self.weights[0]).permute(0, 3, 2, 1).squeeze(-1)).permute(0, 2,
                                                                                                                1).unsqueeze(
                1)
            x_complex = self.complex_conv(x * self.weights[1])
...
#         self.dense_decoder = AdaptiveDecoder(h, h.n_fft // 2 + 1, f_types="P", num_channel=h.dense_channel)
