...
# from estoi import stoi as estoi_metriclsd
...
def compute_metrics(cleanFile, enhancedFile, Fs, path):
    ...
    sisnr = si_snr(data1,data2,sampling_rate1)
    # compute the pesq
...
def compute_all_metrics(cleanFile, enhancedFile, Fs, path):
    alpha = 0.95
    ...
    return pesq_mos, CSIG, CBAK, COVL, segSNR, sisnr,sisdr,STOI
...
def si_snr(clean_speech, processed_speech, sample_rate):
    # Check the length of the clean and processed speech. Must be the same.
    ...
    si_snr = 10 * np.log10(np.sum(np.linalg.norm(clean_speech))**2 / np.sum(np.linalg.norm(clean_speech - processed_speech)**2))
    return si_snr
