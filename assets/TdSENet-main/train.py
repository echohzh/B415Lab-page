...
class Trainer:
    def __init__(self, train_ds, test_ds,):
        ...
        self.model = TdNet(num_channel=64, num_features=self.n_fft // 2 + 1).to(self.device)
    ...
    def train_step(self, batch):
        ...
        # loss += args.loss_weights[5] * content_loss
        loss.backward()
        ...
        # if content_loss !=0:
        #     return loss.item(), discrim_loss_metric.item(),  loss_mag.item(), loss_ri.item(), loss_pha.item(),time_loss.item(),loss_cp.item(), gen_loss_GAN.item(),content_loss.item()
        # else:
    ...
    def test_step(self, batch):
        ...
        # noise_real, noise_imag = noisy_spec[:, 0, :, :].unsqueeze(1) - clean_real, noisy_spec[:, 1, :, :].unsqueeze(
        #     1) - clean_imag
    ...
    def train(self):
        ...
        for epoch in range(max(0, last_epoch),args.epochs):
            ...
            for idx, batch in enumerate(self.train_ds):
                ...
                #     avg_disc_loss = sum(self.disc_loss_history[-50:]) / 50
                #     if avg_loss + avg_disc_loss < threshold:
                #         logging.info(
                #             f"Early stopping at step {step} in epoch {epoch} due to low average loss and disc_loss {avg_loss + avg_disc_loss}.")
                #         # 触发早停后降低阈值
                #         threshold -= 0.01
                #         current_step = step
                #         break
...
def main():
    ...
    # train_ds, test_ds = dataloader.load_base_data(args.data_dir, args.noisy_dir, args.batch_size, 10, args.cut_len)
    train_ds, test_ds = dataloader.load_data(args.data_dir, args.batch_size, 8, args.cut_len)
